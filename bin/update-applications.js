...
types.forEach((type) => {
  ...
  request(`https://raw.githubusercontent.com/WhichBrowser/Parser/master/data/applications-${type.name.toLowerCase()}.php`,
    (err, response = {}) => {
      if (err) {
        return;
      }
      ...
      // Remove => and add :
      result = result.replace(/\[(.*)]/g, '{$1}').replace(/'(.*?)'.*?=>.*?('.*?'|\d+|Constants[\w\\:]*[A-Z]|true|false)/g, '$1: $2');
